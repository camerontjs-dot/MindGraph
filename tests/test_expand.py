...
class TestExpandResults:
    ...
    def test_direct_expand_preserves_seed_scope_warning(
        self, expand_db, expand_embedder
    ):
        conn = db.get_db(expand_db)
        try:
            phase_2 = run_query(
                conn,
                "current alpha status this week",
                expand_embedder,
                final_top_k=1,
            )
            expanded = expand_results(conn, phase_2, depth=1, expand_top_k=20)
        finally:
            conn.close()

        assert expanded
        for row in phase_2 + expanded:
